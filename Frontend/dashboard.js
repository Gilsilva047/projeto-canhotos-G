...
// Função principal que roda apenas quando a página está totalmente carregada
document.addEventListener('DOMContentLoaded', async () => {
    
    // VERIFICA AUTENTICAÇÃO PRIMEIRO
    const authResult = initializeAuthAndUserDisplay();
    if (!authResult.isAuthenticated) {
        // Se não estiver autenticado, a função initializeAuth já faz o logout.
        // Nada mais precisa ser executado.
        return;
    }

    // --- VARIÁVEIS GLOBAIS E ELEMENTOS DA UI ---
    // Todos os getElementById são chamados aqui dentro, garantindo que a página já carregou.
    ...
    let currentUser = {
        ...
        isMasterAdmin: sessionStorage.getItem('isMasterAdmin') === 'true'
    };
    ...
    function renderCanhotos(uploads) {
        ...
        uploads.forEach(canhoto => {
            ...
            card.innerHTML = `
                <div class="card-image-wrapper">${imageContent}</div>
                <div class="card-info">
                    <div>
                        <h3>NF: ${canhoto.nf}</h3>
                    ...
                    <div class="card-footer">
                        <button class="action-btn primary download-btn"><i class="fas fa-download"></i> Baixar</button>
                    </div>
                </div>`;
